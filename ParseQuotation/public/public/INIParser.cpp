...
string INIParser::GetValue(string root, string key, string sDefault)
{
...
vector<ININode>::size_type INIParser::SetValue(string root, string key, string value)
{
	...
	if (map_ini.end() != itr)
	{
		map<string, string>::iterator iter_sub = itr->second.sub_node.find(key);
		if (itr->second.sub_node.end() != iter_sub)
		{
			iter_sub->second = value;
		}
		else
		{
			itr->second.sub_node.insert(pair<string, string>(key, value));
			//itr->second.sub_node.InsertElement(key, value);
		}
	}
	else
	{
